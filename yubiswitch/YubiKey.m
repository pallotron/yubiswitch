...
#import "YubiKey.h"
// This class is responsible for communicating with the helper process, which itself controls the USB device
...
-(id)init {
    if (self = [super init]) {
        ...
        NSError *error = nil;
        if (![self blessHelperWithLabel:@"com.pallotron.yubiswitch.helper" error:&error]) {
            [self raiseAlertWindow:[NSString stringWithFormat:@"Failed to bless helper. Error: %@", error]];
            exit(EXIT_FAILURE);
        }
        [self disable];
    }
